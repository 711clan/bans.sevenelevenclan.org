...
function __stats() {
	...
	if($_SESSION["loggedin"] && !($_SESSION["logg3din"])) {
		$active	    = @mysql_num_rows( @mysql_query("SELECT `bid` FROM `".$config->db_prefix."_bans` WHERE ((`ban_created` + (`ban_length` * 60)) > ".time()." OR `ban_length` = '0')") );
		$admins	    = @mysql_num_rows( @mysql_query("SELECT `id` FROM `".$config->db_prefix."_amxadmins`") );
		$servers    = @mysql_num_rows( @mysql_query("SELECT `id` FROM `".$config->db_prefix."_serverinfo`") );
		...
		if( mysql_num_rows($checkQry) > 0 ) {
			...
			@mysql_query("INSERT INTO `statistics` (`bans`,`admins`,`servers`,`last_update`,`webserver`,`path`,`url`) VALUES ('$active', '$admins', '$servers', ".time().", '".$_SERVER['SERVER_ADDR']."', '$config->document_root', '".$_SERVER['SERVER_NAME']."')", $stat_connect);
		}
		@mysql_close($stat_connect);

		$_SESSION["logg3din"] = true;
	}
}
