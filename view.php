 ...
 *
 */
/*$stats[kicks]        = mysql_num_rows( mysql_query("SELECT bid FROM ".$config->db_prefix."_bans WHERE ban_kicks > 0") ); */
$stats_query = mysql_query("SELECT SUM(ban_kicks) AS kick_sum FROM ".$config->db_prefix."_bans");

if( !$stats_query ) die( 'Error running query: ' . mysql_errno( $stats_query ) . ' ' . mysql_error( $stats_query ) );

$stats_row = mysql_fetch_assoc( $stats_query );
$stats[kicks] = $stats_row['kick_sum'];  
$kicksonly = $stats.kicks > 0;
